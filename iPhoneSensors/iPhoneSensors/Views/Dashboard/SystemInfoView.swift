            ...
            ScrollView {
                AdaptiveCardGrid(spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        DataRow(label: locManager.t("label.screenScale"), value: LocalizedDisplayValue.numberNoSpace("%.0f", sys.screenScale, unitKey: "unit.scale", localization: locManager), icon: "arrow.up.left.and.arrow.down.right")
                        DataRow(label: locManager.t("label.brightness"), value: LocalizedDisplayValue.numberNoSpace("%.0f", sys.screenBrightness * 100, unitKey: "unit.percent", localization: locManager), icon: "sun.max.fill")
                        Divider().opacity(0.3)
                    ...
                    NavigationLink {
                        ...
                        HStack {
                            ...
                            Text(locManager.t("label.processorDetails"))
                                .foregroundStyle(colorScheme == .dark ? .white : .primary)
                            Spacer()
                            Text("\(sys.activeProcessorCount) \(locManager.t("unit.cores"))")
                                .foregroundStyle(.secondary)
                            Image(systemName: "chevron.right")
                                .foregroundStyle(.tertiary)
                        }
