    ...
    var body: some View {
        ...
        SOVariant(sensor: "THERMAL", accent: SO.thermAccent) {
            VStack(spacing: 0) {
                ...
                SOTextLabel("ILLUSTRATIVE · iOS does not expose absolute °C")
        ...
        SOVariant(sensor: "MEM", accent: SO.memAccent) {
            VStack(spacing: 0) {
                ...
                Text(ByteCountFormatter().string(fromByteCount: Int64(total * level)))
                    ...
                    .foregroundStyle(.white)
                SOFormattedText("USED OF %@", ByteCountFormatter().string(fromByteCount: Int64(total)))
...
private struct CpuCoreGrid: View {
    ...
    var body: some View {
        SOVariant(sensor: "CPU", accent: SO.cpuAccent) {
            VStack(spacing: 0) {
                Spacer().frame(height: 80)
                SOFormattedLabel(format: "%d cores · per-core load", value: sys.activeProcessorCount)
                Spacer().frame(height: 18)
                grid.frame(width: 320, height: 320)
                Spacer().frame(height: 14)
                SOTextLabel("4 EFFICIENCY · 4 PERFORMANCE")
                    .font(.system(size: 11, weight: .heavy)).tracking(2)
                    .foregroundStyle(.white.opacity(0.5))
                Spacer()
